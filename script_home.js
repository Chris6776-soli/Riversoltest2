...
// Upload button click (MOVE OUTSIDE)
...
try {
  ...
  console.warn("URL fetch failed, but upload worked:", e);
}

    let contact = "";
    let contactType = "";

    if (clientPhone.style.display !== "none") {
      contact = clientPhone.value.trim();
      contactType = "phone";
    } else {
      contact = clientEmail.value.trim();
      contactType = "email";
    }
  ...
  alert(err.message); // 👈 shows exact Firebase error
    submitUpload.disabled = false;
    submitUpload.textContent = "Upload";
  }
};
